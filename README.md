...
## 自动化
- T1 每日 02:20 巡检 + 写日报(.Log/巡检-社会-YYYYMMDD.md)
- T4 每日 02:00 检查项目并更新 .plan/YYYYMMDD.md(9/2~9/17 连续 16 日停摆,本轮 T5 应急补建 plan)
- T5 每日 03:20 完成小步开发并 commit + push(2h 偏移,因 T1 02:20 完成后留 1h 缓冲)
...
- **Phase 0 #6 cron 入口补完(9/8)**:新增 `scripts/run_daily_report.sh` 飞书日报 cron 入口包装(默认 report_date=昨日 CST,输出 `.Log/日报-YYYYMMDD.md`,`--strict` 模式 + `--dry-run` 预览 + `--push-stub` 飞书推送占位),把 `daily_report.py` 脚本骨架升到"可定时调起"形态,真飞书 webhook 推送留给后续周期;手测 `bash scripts/run_daily_report.sh --dry-run` 输出 2026-09-07 Top 10 完整预览,默认模式写入 `.Log/日报-20260907.md`(1454 字节,15 议题 / 13 快照)
  ...
  - 2026-09-14 续(T5 应急补建 9/14 周期 · README 状态对账 + .gitignore bug 修复 · 巡检项 5 接力):T1 巡检时间描述从"01:00"同步为"02:20"(8/25 起的 20 个 T1 实际触发时点均落在 02:20±2min 窗口,README 早期 01:00 描述系 8/25 之前设定,从未同步;9/13 静默为唯一例外,9/14 02:21 恢复 commit `abb801a`);T4 调度连续 13 日停摆 9/2→9/14 在本条目附记;状态日期 2026-09-12 → 2026-09-14;**额外修复**:9/5 commit `56f9886` 提交 `.plan/   # ...` 模式因尾注释导致 gitignore pattern 失效(T5 9/5 周期工作树 clean 之所以成立,是因为 9/5~9/13 期间 T4 调度停摆从未创建过 plan markdown 文件,本轮 03:20 应急补建 `.plan/20260914.md` 才首次触发该 bug 暴露);修正方案:`.plan/` 单独成行,注释移到上一行 — `git check-ignore -v .plan/20260914.md` 从 exit=1 → exit=0;`项目开发计划.md` 第 231 行后同步追加 9/14 续条目;**Phase 0 严格计数仍 7/6**(本轮属"状态对账 + 模式修复"通道,非新主任务;巡检项 5 接力闭环 + 9/5 .plan/ 模式根因修复真正闭环)
  - 2026-09-15 续(T5 应急补建 9/15 周期 · Phase 0 #4 质量层 · 工具链再扩展 · 工程侧 dashboard 维度):新增 `scripts/phase_status.py` 项目阶段状态自动报告 CLI(对齐 `issue_summary.py` + `issue_trend.py` 风格,`argparse` + 退出码 + `--json`/`--strict` 2 档开关;无 PyYAML 之外的额外依赖);聚合项目全景 7 段指标:① 数据文件清单 ② 议题库 quick stats ③ 关系图 quick stats ④ 声量快照 quick stats ⑤ Phase 进度(grep `项目开发计划.md` 自动算 done/todo:Phase 0 done=6 todo=0 / Phase 1 done=0 todo=7)⑥ 工具链索引(扫 scripts/ 下所有 .py + .sh 抓"用途")⑦ 健康检查(议题 id 唯一 / 关系图 0 孤儿边 / 声量外键 100% 命中 / 情感加和 1.0±0.01 / 热度 0-100 范围,5 维全 ✅);`--strict` 0 健康问题 exit 0,`--issues /tmp/missing.yaml` 缺失文件 exit 1;与 `issue_summary.py`(库/关系图静态盘点)+ `issue_trend.py`(时序趋势)+ `issue_relations_analysis.py`(图论深度)+ `validate_pulse.py`(声量校验)互补,本脚本专注"工程侧 dashboard 快照",为 T5 写代码前 + T1 巡检后的人工快速核对提供 CLI 入口;**Phase 0 严格计数仍 7/6**(本轮属 #4 质量层"工程侧状态聚合"维度扩展,非新主任务);工具链至此 10 段齐全:md_to_json + validate_pulse + issue_summary + issue_trend + issue_relations_analysis + daily_report + feishu_publish + run_daily_report.sh + register_feishu_daily.sh + phase_status
  - 2026-09-16 续(T5 应急补建 9/16 周期 · Phase 0 #4 质量层 · 工具链再扩展 · 多议题横向对比维度):新增 `scripts/issue_compare.py` 多议题对比 CLI(对齐 `issue_summary.py` + `issue_trend.py` + `issue_relations_analysis.py` 风格,`argparse` + 退出码 + `--json`/`--strict`/`--source`/`--top K` 4 档开关 + Markdown/JSON 双输出;≥ 2 议题 ID positional args;退出码 0/1/2/3 五档);聚合 `data/issues.yaml` × `data/pulse_snapshots.yaml` × `data/issue_relations.yaml`,产出 8 段指标:① 基础 ② 热度 ③ 情感 ④ 关系 ⑤ category 分组 ⑥ 极值定位(heat_top issue-005=95 / volume_top issue-005=3880 / neg_deepest issue-002=0.530 / degree_top issue-001=6)⑦ 排名 Top K(4 档 heat/volume/neg/degree 倒序)⑧ 缺失议题 ID 列表;手测 issue-001/002/005 三议题对比输出 OK,填补 Phase 0 #4 任务 #4 第二步"多议题横向对比 `/pulse/compare?ids=issue-001,issue-002`"未触段;**Phase 0 严格计数仍 7/6**(本轮属 #4 质量层"多议题横向对比"维度扩展,非新主任务);工具链至此 11 段齐全;9/16 `.plan/20260916.md` 因 T4 调度连续 16 日停摆仍未生成(9/2→9/16),本轮按 9/14/9/15 同款"应急补建"通道直接落地变更,不擅自建 plan 文件
  - 2026-09-17 续(T5 应急补建 9/17 周期 · Phase 0 #6 调度层健康检查维度):新增 `scripts/scheduler_health.py` 调度层健康检查 CLI(仅 stdlib,无 PyYAML / 无 mavis CLI 依赖;`argparse` + 退出码 + `--days`/`--date`/`--json`/`--strict` 4 档开关);聚合调度层 4 段检查:① `.Log/巡检-社会-YYYYMMDD.md` T1 巡检(02:20 窗口)连续性 ② `.plan/YYYYMMDD.md` T4 plan 生成(02:00~03:00 窗口)连续性 ③ `.Log/日报-YYYYMMDD.md` 09:00 飞书日报(数据日期=昨日)连续性 ④ `scripts/register_feishu_daily.sh` 09:00 cron 注册脚本完整性(文件存在 + 大小 + 关键字符串"feishu-社会-日报"+"0 9 * * *"+"Asia/Shanghai");每段输出:应到/实到/连续缺失/最近一份日期/状态(✅/⚠️/❌);退出码 0 全 OK / 1 任一段 ❌(连续缺失 ≥ 2 日)/ 2 缺依赖(本脚本无依赖,留档)/ 3 参数错;9/17 02:20 巡检时点手测 `--days 14` 输出:段 1 T1 巡检 14/13(9/13 单日静默 ⚠️)/段 2 T4 plan 14/0 全停摆 ❌/段 3 09:00 日报 14/4 连续停摆 ❌/段 4 cron 脚本完整 ✅,整体 ❌;填补 Phase 0 #6"调度层健康"工具缺口(此前 `phase_status.py` 关注代码/数据完整性,本脚本专注调度层完整性 = 4 段 × 应到/实到/连续缺失 + 单日静默允许 ⚠️ / ≥2 日连续缺失 ❌ 双档判定);**Phase 0 严格计数仍 7/6**(本轮属 #6 飞书日报"调度层"健康检查维度扩展,非新主任务);工具链至此 12 段齐全:md_to_json + validate_pulse + issue_summary + issue_trend + issue_relations_analysis + phase_status + issue_compare + scheduler_health + daily_report + feishu_publish + run_daily_report.sh + register_feishu_daily.sh;9/17 `.plan/20260917.md` 因 T4 调度连续 17 日停摆仍未生成(9/2→9/17),本轮按 9/14/9/15/9/16 同款"应急补建"通道直接落地变更,不擅自建 plan 文件
- **Phase 1**: 0/7 待启动(Web App 骨架未搭建,连续 24 个 T5 潜在窗口未启动,9/17 接力观察)
...
- 调度层健康检查:`python3 scripts/scheduler_health.py`(4 段:T1 巡检连续性 / T4 plan 连续性 / 09:00 飞书日报连续性 / cron 注册脚本完整性;`--days N` 回看 N 日,默认 14;`--date YYYY-MM-DD` 指定参照日期便于测试;`--json` 机器可读;`--strict` 任何缺口 ≥ 2 日 → exit 1)
